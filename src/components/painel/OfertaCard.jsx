...
/**
 * Card de produto/serviço no grid — visual product-card (inspiração Gogo, UI própria).
 */
    ...
    >
      <div className="relative aspect-[4/3] max-h-[240px] overflow-hidden bg-border/40">
        ...
        {badge ? (
          <span
            role="status"
            className="absolute top-3 left-3 rounded-lg bg-accent px-2 py-0.5 text-xs font-semibold text-white shadow-sm dark:text-bg"
          >
            {badge}
          </span>
        ) : null}
      ...
      >
        ...
        {preco ? (
          <span className="mt-auto flex flex-wrap items-baseline gap-2 pt-3">
            <span className="text-base font-semibold text-accent">{preco}</span>
            {precoDe ? (
              <span className="text-sm text-muted line-through">{precoDe}</span>
            ) : null}
          </span>
        ) : null}
      </button>
